 ...
 *
 * @example
 *   getPolynom(12, 4,2,3,5) => y = 12*x^4 + 4*x^3 + 2*x^2 + 3*x + 5
...
function getPolynom(...args) {
  return args.length
    ? (x) => args.slice(0).reverse().map((el, i) => el * (x ** i)).reduce((a, b) => a + b)
    : null;
}
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((it) => JSON.stringify(it)).join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${args.map((it) => JSON.stringify(it)).join(',')}) ends`);
    return result;
  };
}
