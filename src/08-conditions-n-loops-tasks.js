...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(index) === i && str.indexOf(index, i + 1) === -1) {
      return index;
    }
  }
  return null;
...
function reverseString(str) {
  return str.split('').map((element) => element.split('').reverse()).reverse().join('');
}
...
function getDigitalRoot(num) {
  const newNumber = `${num}`.split('').reduce((prev, acc) => +prev + +acc);
  return newNumber > 10 ? getDigitalRoot(newNumber) : newNumber;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (opening.includes(str[i])) {
      result.push(str[i]);
    } else if (closing.includes(str[i]) && result[result.length - 1]
    === opening[closing.indexOf(str[i])]) {
      result.pop();
    } else {
      return false;
    }
...
function getMatrixProduct(m1, m2) {
  ...
  return result.map((row, i) => row
    .map((val, j) => m1[i].reduce((sum, el, k) => sum + (el * m2[k][j]), 0)));
}
